...
if ( ! function_exists( 'bf_is_doing_ajax' ) ) {
	...
	function bf_is_doing_ajax( $ajax_action = '' ) {
		...
		if ( empty( $ajax_action ) ) {
			return $is_ajax;
		} elseif ( ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] === $ajax_action ) { // support for WP ajax action
			return TRUE;
		} elseif ( ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] === 'bf_ajax' &&
		           ! empty( $_REQUEST['reqID'] ) && $_REQUEST['reqID'] === $ajax_action
...
if ( ! function_exists( 'bf_render_css_block_array' ) ) {
	...
	function bf_render_css_block_array( $block, $value ) {
		...
		if ( isset( $block['before'] ) && is_string( $value ) ) {
