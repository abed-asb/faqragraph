...
class BF_Shortcode {
	...
	public function can_register_vc_add_on() {
		...
		} else if ( function_exists( 'vc_is_inline' ) && vc_is_inline() ) { // Fix for vc inline editor
			$register = TRUE;
		}
	...
	public function transform_to_vc_fields( &$fields ) {
		...
		$replacements = array( // 'bf key' => 'vc key'
		...
		foreach ( $fields as $idx => $field ) {
			...
			} else if ( in_array( $field['type'], $vc_fields ) ) {
	 ...
	 * @type array  $scripts        dedicated scripts for the shortcode. array like bf_enqueue_tinymce_style() return values
	 * @type array  $style          dedicated styles  for the shortcode. array like bf_enqueue_tinymce_style() return values
	 *
